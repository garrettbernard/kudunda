...
if (!$error && ($size[0] < 25) || ($size[1] < 25))
{
	$error = 'Please upload an image bigger than 25px.';
 		...
 		$sql    = "SELECT * FROM pictures WHERE uploader_id = {$_GET['uid']} ORDER BY id DESC";
		$result = mysql_query($sql, $link);
		$top_picture_number = mysql_fetch_array($result);
		$numrow = ($top_picture_number[0] + 1);
		
// Processing
	$ext = substr(strrchr($_FILES['Filedata']['name'], '.'), 1);
	$filename = ($_GET['uid'] . '_' . $numrow . '_' . $_GET['eid'] . '.' . $ext);
// Its a demo, you would move or process the file like:

 // move_uploaded_file($_FILES['Filedata']['tmp_name'], 'c:/apache2/htdocs/kudunda/pictures/' . $_FILES['Filedata']['name']);
move_uploaded_file($_FILES['Filedata']['tmp_name'], $absolute_path . 'pictures/' . $filename);
$uid = $_GET['uid'];
$eid = $_GET['eid'];

	
 // $return['src'] = 'c:/apache2/htdocs/kudunda/pictures/' . $_FILES['Filedata']['name'];
 $return['src'] = $absolute_path . 'pictures/' . $filename;
...
if ($error) {
	...
	if ($return['status'] != '0') {
	$sql	= "INSERT INTO pictures (id, uploader_id, event_id, filename, keywords) VALUES ('$numrow', '$uid', '$eid', '$filename', NULL)";
	mysql_query($sql, $link) or die(mysql_error());  
	}
 
	// Our processing, we get a hash value from the file
	// $return['hash'] = md5_file($_FILES['Filedata']['tmp_name']);
	// $return['hash'] = md5_file('c:/apache2/htdocs/kudunda/pictures/' . $_FILES['Filedata']['name']);
	$return['hash'] = md5_file($absolute_path . 'pictures/' . $filename);
 
	// ... and if available, we get image data
	// $info = @getimagesize($_FILES['Filedata']['tmp_name']);
	// $info = @getimagesize('c:/apache2/htdocs/kudunda/pictures/' . $_FILES['Filedata']['name']);
	$info = @getimagesize($absolute_path . 'pictures/' . $filename);
